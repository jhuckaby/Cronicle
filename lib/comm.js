...
module.exports = Class.create({
	...
	checkServerClock: function(slave) {
		...
		var max = this.server.config.get('max_clock_drift') || 10;
		
		if ((drift >= max) && (slave.clock_drift < max)) {
	...
	handleNewSocket: function(socket) {
		...
		socket.on('authenticate', function(params) {
			...
			if (params.master_hostname && params.now && params.token) {
				...
				// Allow operators to accommodate unavoidable clock skew, while preserving
				// the historical 60-second limit for existing configuration files.
				var auth_window = parseInt( self.server.config.get('remote_server_auth_window'), 10 );
				if (!isFinite(auth_window) || (auth_window < 0)) auth_window = 60;
				if (Math.abs(Tools.timeNow() - params.now) > auth_window) {
				...
				self.storage.get( 'sessions/' + params.token, function(err, session) {
					if (err) {
						...
						// Bind the current user record to the socket so future broadcasts can
						// enforce recipient-specific object authorization before emission.
						self.storage.get( 'users/' + self.usermgr.normalizeUsername(session.username), function(err, user) {
							if (err || !user || !user.active) {
								self.logError('socket', "Socket client " + socket.id + " failed to load its user (IP: "+ip+")");
								socket.emit( 'auth_failure', { description: "User not found or inactive." } );
								return;
							}
							self.logDebug(4, "Socket client " + socket.id + " has authenticated via user session (IP: "+ip+")");
							socket._pixl_user = user;
							socket._pixl_auth = true;
						} );
					}
				} );
