...
module.exports = Class.create({
	...
	launchJob: function(event, callback) {
		...
		function(err) {
			...
			if (event.max_children) {
				...
				if (event_jobs.length >= event.max_children) {
					// too many event children running
					return callback( new Error("Maximum of "+event.max_children+" "+Tools.pluralize("job", event.max_children)+" already running for event: " + event.title) );
				}
			}
			...
			if (!servers.length && server_group) {
				...
				if (event.multiplex) {
					...
					// run on one server in group, chosen at random
					servers.push( Tools.randArray(candidates) );
				}
			} // find slave
			...
			for (var idx = 0, len = servers.length; idx < len; idx++) {
				...
				// fire web hook
				var stub = Tools.mergeHashes( job, { action: 'job_start' } );
				if (job.web_hook) {
					self.logDebug(9, "Firing web hook for job start: " + job.id + ": " + job.web_hook);
					self.request.json( job.web_hook, stub, function(err, resp, data) {
						// ignore response
					} );
				}
				if (self.server.config.get('universal_web_hook')) {
					self.logDebug(9, "Firing universal web hook: " + self.server.config.get('universal_web_hook'));
					self.request.json( self.server.config.get('universal_web_hook'), stub, function(err, resp, data) {
						// ignore response
					} );
				}
	...
	launchLocalJob: function(job) {
		...
		if (this.multi.master || job.detached) {
			fs.writeFile( job.log_file.replace(/\.log$/, '.json'), JSON.stringify(job) );
		}
		...
		if (!child_opts.gid.toString().match(/^\d+$/)) {
			try {
				...
				job.pid = 0;
				job.code = 1;
				job.description = "Plugin Error: Group does not exist: " + child_opts.gid;
				this.logError("child", job.description);
				this.activeJobs[ job.id ] = job;
				this.finishLocalJob( job );
				return;
		...
		if (job.detached) {
			...
			child_opts.stdio = ['ignore', fs.openSync(job.log_file, 'a'), fs.openSync(job.log_file, 'a')];
			...
			// attach streams
			child_opts.stdio = ['pipe', 'pipe', fs.openSync(job.log_file, 'a')];
			...
			this.logDebug(3, "Spawned child process: " + job.pid + " for job: " + job.id, job.command);
	...
	uploadJobLog: function(job, callback) {
		...
		if (this.multi.master) {
			...
			fs.readFile( job.log_file, { encoding: null }, function(err, data) {
				// data will be a Buffer
				if (err) data = new Buffer("(Empty log file)\n");
				
				self.storage.put( path, data, function(err) {
					if (err) {
					...
					if (self.server.config.get('copy_job_logs_to')) {
						var dest_dir = self.server.config.get('copy_job_logs_to').replace(/\/$/, '') + '/';
						self.logDebug(9, "Moving local file: " + job.log_file + " to: " + dest_dir);
						
						self.logger.rotate( job.log_file, dest_dir, function(err) {
							if (err) {
								self.logError('file', "Failed to move local job log file: " + job.log_file + ": " + err);
								fs.unlink( job.log_file, function(err) {;} );
							}
							else {
								self.logDebug(9, "Successfully moved local job log file: " + job.log_file + ": " + dest_dir);
							}
	...
	fetchStoreJobLog: function(job) {
		...
		var api_url = this.getServerBaseAPIURL( slave.ip || slave.hostname ) + '/app/fetch_delete_job_log';
		...
		this.request.get( api_url, function(err, resp, data) {
			// check for error
			if (err) {
				...
				fs.writeFile( job.log_file, data, function(err) {
					if (err) {
						var err_msg = "Failed to store job log file locally: " + job.log_file + ": " + err;
						self.logError('job', err_msg);
						return;
					}
	...
	finishJob: function(job) {
		...
		this.storage.expire( log_path, Tools.timeNow(true) + (86400 * this.server.config.get('job_data_expire_days')) );
		...
		// fire web hook
		if (job.web_hook) {
			this.logDebug(9, "Firing web hook for job: " + job.id + ": " + job.web_hook);
			this.request.json( job.web_hook, Tools.mergeHashes(data, { action: 'job_complete' }), function(err, resp, data) {
				// ignore response
			} );
		}
		if (this.server.config.get('universal_web_hook')) {
			this.logDebug(9, "Firing universal web hook for job: " + job.jd + ": " + this.server.config.get('universal_web_hook'));
			this.request.json( this.server.config.get('universal_web_hook'), Tools.mergeHashes(data, { action: 'job_complete' }), function(err, resp, data) {
				// ignore response
			} );
		}
		...
		if (job.catch_up && !this.schedulerGraceTimer && !this.schedulerTicking && (dargs.sec != 59)) {
	...
	monitorServerResources: function(callback) {
		...
		cp.exec( '/bin/ps -eo "ppid pid %cpu rss"', { timeout: 10 * 1000 }, function(err, stdout, stderr) {
			if (err) {
			...
			for (var id in self.activeJobs) {
				...
				// only match jobs that have been running for more than 10 seconds
				// this way we don't record cpu/mem for a process that is just starting up
				if (pids[ job.pid ] && (now - job.time_start >= 10)) {
					...
					if (self.debugLevel(9)) {
						self.logDebug(9, "Active Job: " + job.pid + ": CPU: " + cpu + "%, Mem: " + Tools.getTextFromBytes(mem));
					}
	...
	watchJobLog: function(args, socket) {
		...
		if (!self.activeJobs[args.id]) {
			self.logError('socket', "Could not locate active job: " + args.id);
			return;
		}
		
		// prepare to log watch
		var job = self.activeJobs[args.id];
		var log_file = job.log_file;
		var log_fd = null;
		var log_stats = null;
		var log_chunk_size = 32678;
		var log_buffer = new Buffer(log_chunk_size);
		var log_pos = 0;
