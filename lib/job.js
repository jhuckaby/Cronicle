...
module.exports = Class.create({
	...
	monitorServerResources: function(callback) {
		...
		var cmd = this.server.config.get('ps_monitor_cmd');
		// FreeBSD's ps uses BSD flags to select all processes, including those without a terminal.
		if (!cmd) cmd = (process.platform == 'freebsd') ?
			'/bin/ps -ax -o "ppid pid %cpu rss"' : '/bin/ps -eo "ppid pid %cpu rss"';
