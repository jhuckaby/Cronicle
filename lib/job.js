...
module.exports = Class.create({
		...
		switch (event.algo || 'random') {
			...
			case "least_cpu":
				...
				for (var hostname in servers) {
					cpus[hostname] = servers[hostname].data.cpu || 0;
				}
			...
			case "least_mem":
				...
				var servers = this.getAllServers();
				for (var hostname in servers) {
					mems[hostname] = servers[hostname].data.mem || 0;
				}
				var jobs = this.getAllActiveJobs();
				for (var job_id in jobs) {
					var job = jobs[job_id];
	...
	monitorServerResources: function(callback) {
		...
		var job_startup_grace = this.server.config.get('job_startup_grace') || 10;
		
		this.logDebug(9, "Checking server resources: " + cmd);
