...
module.exports = Class.create({
	...
	launchJob: function(event, callback) {
		...
		// check dependencies
		if (!this.checkJobDependencies(event)) {
			return callback( new Error("Dependencies not met for event: " + event.title) );
		}
	...
	finishJob: function(job) {
		...
		this.checkDependentJobs( job.event );
	...
	checkJobDependencies: function(event) {
		// check if event dependencies are met
		if (!event.dependencies) return true;
		
		var self = this;
		var deps = event.dependencies;
		var jobCodes = this.state.jobCodes || {};
		
		// Replace success(id)
		deps = deps.replace(/success\(([\w\-]+)\)/g, function(m, id) {
			return (jobCodes[id] === 0) ? "true" : "false";
		});
		
		// Replace failure(id)
		deps = deps.replace(/failure\(([\w\-]+)\)/g, function(m, id) {
			return (jobCodes[id] > 0) ? "true" : "false";
		});
		
		// Replace done(id)
		deps = deps.replace(/done\(([\w\-]+)\)/g, function(m, id) {
			return (id in jobCodes) ? "true" : "false";
		});
		
		// Replace & with &&, | with ||
		deps = deps.replace(/&/g, '&&').replace(/\|/g, '||');
		
		// Sanitize: only allow true, false, &&, ||, !, (, ), spaces
		if (deps.match(/[^truefals\&\| \!\(\)]/)) {
			this.logDebug(3, "Invalid characters in dependency expression: " + event.dependencies);
			return false; 
		}
		
		try {
			var result = eval(deps);
			return !!result;
		}
		catch (e) {
			this.logDebug(3, "Error evaluating dependency expression: " + event.dependencies + ": " + e.message);
			return false;
		}
	},

	checkDependentJobs: function(completed_event_id) {
		// find all events that depend on the one that just completed
		var self = this;
		...
		this.storage.listGet( 'global/schedule', 0, 0, function(err, items) {
			...
			items.forEach( function(item) {
				if (item.dependencies && item.enabled) {
					// Check if this item depends on the one that just completed
					var re = new RegExp("\\b" + completed_event_id + "\\b");
					if (re.test(item.dependencies)) {
						if (self.checkJobDependencies(item)) {
							self.logDebug(4, "Dependencies met for event: " + item.title + ", launching now");
							var job = Tools.mergeHashes(item, {
								source: "Dependency (" + completed_event_id + ")"
							});
							self.launchOrQueueJob( job, function(err, jobs_launched) {
								if (err) self.logDebug(3, "Failed to launch dependent job: " + item.title + ": " + err.message);
							} );
						}
					}
				}
