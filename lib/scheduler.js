						...
						function (err) {
							if (err) {
								...
								if (!catch_up_only && (Tools.timeNow(true) - cursor <= 30) && !err_msg.match(/(Category|Plugin).+\s+is\s+disabled\b/) && !launches[item.id]) {
									...
									// possibly fire chain reaction here
									if (item.chain_error) {
										// create fake job object to pass along
										var fake_job = Tools.mergeHashes(item, {
											code: 'job_launch_failure',
											description: (err.message || err),
											event_title: item.title,
											event: item.id,
											id: ''
										});
										self.chainReaction(fake_job, item.chain_error);
									}
