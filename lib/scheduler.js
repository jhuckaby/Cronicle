...
module.exports = Class.create({
				...
				async.eachSeries( items, async.ensureAsync( function(item, callback) {
					...
					async.whilst(
						...
						async.ensureAsync( function (callback) {
							...
							if(self.holidayCal.has(item.timezone || self.tz))
								moment.updateLocale('us', self.holidayCal.get(item.timezone || self.tz));
							var margs = moment(cursor * 1000).tz(item.timezone || self.tz);
							
							if (item.timing && self.checkEventTimingMoment(item.timing, margs, item.timezone || self.tz, item.dateCalcType, item.triggerBefore)) {
								// item needs to run!
	...
	checkEventTiming: function(timing, cursor, tz) {
		...
		if(this.holidayCal.has(tz || this.tz))
			moment.updateLocale('us', this.holidayCal.get(tz || this.tz));
		var margs = moment(cursor * 1000).tz(tz || this.tz);
		return this.checkEventTimingMoment(timing, margs, tz || this.tz, 'Calendar', 0);
	},
	
	checkEventTimingMoment: function(timing, margs, timezone, dateCalcType, triggerBefore) {
