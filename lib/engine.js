...
module.exports = Class.create({
	...
	authSocketEmit: function(key, data, server_groups) {
		...
		for (var id in this.sockets) {
			...
			if (data && data.schedule) {
				// Schedule payloads contain complete Event definitions, including
				// Plugin parameters.  Emit a recipient-specific copy only to sockets
				// that are bound to an authenticated user.
				if (!socket._pixl_user) continue;
				var socket_data = Tools.copyHash(data, true);
				socket_data.schedule = this.filterScheduleForUser(
					{ server_groups: server_groups || [] }, socket._pixl_user, data.schedule
				);
	...
	beforeUserLogin: function(args, callback) {
		...
		async.each( ['schedule', 'categories', 'plugins', 'server_groups'], 
			...
			function(err) {
				if (!err) args.resp.schedule = self.filterScheduleForUser(
					{ server_groups: args.resp.server_groups }, args.user, args.resp.schedule
				);
				callback(err);
			}
		); // each
