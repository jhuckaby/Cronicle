...
module.exports = Class.create({
	...
	startup: function(callback) {
		...
		this.server.on('minute', function() {
			self.monitorServerResources( function(err) {
			...
			if (global.gc) {
				self.gcTimer = setTimeout( function() {
					delete self.gcTimer;
					self.logDebug(10, "Forcing garbage collection now");
					global.gc();
					self.logDebug(10, "Garbage collection complete");
				}, 10 * 1000 );
			}
		} );
		...
		this.storage.listGet( 'global/servers', 0, 0, function(err, servers) {
			...
			self.storage.listGet( 'global/server_groups', 0, 0, function(err, groups) {
				...
				for (var idx = 0, len = groups.length; idx < len; idx++) {
					...
					var regexp = new RegExp( group.regexp );
					if (group.master && self.server.hostname.match(regexp)) {
						eligible = true;
	...
	logActivity: function(action, data, args) {
		// add event to activity logs async
		var self = this;
		if (!args) args = {};
		assert( Tools.isaHash(data), "Must pass a data object to logActivity" );
		...
		function(err) {
			...
			async.each( superiors, 
				function(hostname, callback) {
					...
					var api_url = self.getServerBaseAPIURL( server.ip ) + '/app/ping';
	...
	recoverJobLogs: function() {
		...
		var self = this;
		var log_spec = this.server.config.get('log_dir') + '/jobs/*.log';
		...
		this.logDebug(4, "Looking for leftover job logs: " + log_spec);
		...
		// also look for any leftover job JSON files (master server shutdown)
				...
				function(list_path, callback) {
					...
					self.storage.listGetInfo( list_path, function(err, info) {
						...
						if (info.length > max_rows) {
							...
							self.logDebug(3, "List " + list_path + " has grown too long, trimming to max", info);
							self.storage.listSplice( list_path, info.length, info.length - max_rows, null, callback );
						}
						else {
