...
module.exports = Class.create({
	...
	startup: function(callback) {
		...
		this.web.addURIFilter( /\/api\/app\/\w+/, "API Filter", function(args, callback) {
			...
			self.storage.listGet( 'global/server_groups', 0, 0, function(err, items) {
				self.serverGroups = args.server_groups = items || [];
	...
	tick: function() {
		...
		if (this.numSocketClients) {
			...
			this.authSocketEmit( 'status', status, this.serverGroups );
		}
	...
	authSocketEmit: function(key, data, server_groups) {
		// Only emit to authenticated clients
		for (var id in this.sockets) {
			var socket = this.sockets[id];
			if (!socket._pixl_auth) continue;
			
			if (data && (data.schedule || data.active_jobs)) {
				// Master sockets need complete Job status for cluster synchronization.
				// All other sensitive payloads require a bound user and a filtered copy.
				if (socket._pixl_master) {
					socket.emit( key, data );
					continue;
				}
				if (!socket._pixl_user) continue;
				var socket_data = Tools.copyHash(data, true);
				var filter_args = { server_groups: server_groups || [] };
				if (data.schedule) {
					socket_data.schedule = this.filterScheduleForUser(
						filter_args, socket._pixl_user, data.schedule
					);
				}
				if (data.active_jobs) {
					socket_data.active_jobs = this.filterActiveJobsForUser(
						filter_args, socket._pixl_user, data.active_jobs
					);
				}
				socket.emit( key, socket_data );
			}
	...
	updateClientData: function(name) {
		...
		this.storage.listGet( 'global/' + name, 0, 0, function(err, items) {
			...
			if (name == 'server_groups') self.serverGroups = items;
			var data = {};
			data[name] = items;
			
			if (name == 'schedule') {
				// Schedule updates require group definitions so individual-hostname
				// targets can be filtered using the same rules as the API.
				self.storage.listGet( 'global/server_groups', 0, 0, function(err, groups) {
					if (err) {
						self.logError('storage', "Failed to fetch list: global/server_groups: " + err);
						return;
					}
					self.authSocketEmit( 'update', data, groups );
				} );
			}
			else self.authSocketEmit( 'update', data );
		} );
	},
	...
	beforeUserLogin: function(args, callback) {
		...
		async.each( ['schedule', 'categories', 'plugins', 'server_groups'], 
			...
			function(err) {
				if (!err) {
					self.serverGroups = args.resp.server_groups;
					var filter_args = { server_groups: args.resp.server_groups };
					args.resp.schedule = self.filterScheduleForUser(
						filter_args, args.user, args.resp.schedule
					);
					args.resp.activeJobs = self.filterActiveJobsForUser(
						filter_args, args.user, args.resp.activeJobs
					);
				}
				callback(err);
			}
	...
	goMaster: function() {
		...
		// ensure this function only ever gets called once
		if (this.becameMaster) {
			var err_msg = "MASTER CONFLICT: This server is becoming master twice.  Shutting down immediately.  Please file a GitHub issue and include this full log entry.";
			var stack = (new Error()).stack;
			this.logDebug(1, err_msg, { stack });
			this.logError('multi', err_msg, { stack });
	...
	goSlave: function() {
		...
		if (this.becameMaster) {
			var err_msg = "MASTER CONFLICT: This server is being forced to demote itself from master primary to slave.  Shutting down immediately.  Please file a GitHub issue and include this full log entry.";
			var stack = (new Error()).stack;
			this.logDebug(1, err_msg, { stack });
			this.logError('multi', err_msg, { stack });
			this.server.shutdown();
