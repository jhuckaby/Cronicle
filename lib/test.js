...
config.base_app_url = "http://localhost:3012";
...
// create an api key that the unit test will use
/*var unit_key = "11111111111111111111111111111111";

setup.storage.push([ "listPush", "global/api_keys", {
	"id": "unittestkey",
	"key": unit_key,
	"title": "Unit Test App",
	"active": 1,
	...
	"modified": 1434125333,
	"created": 1434125333,
	"description": "For unit test use only.",
	"privileges": { "admin": 1 }
} ] );*/
...
module.exports = {
	...
	setUp: function (callback) {
		...
		try { cp.execSync('rm -rf logs/unit.log logs/unit.pid data/unittest'); }
		catch (e) {;}
	...
	tests: [
		...
		function testAPICreateCategory(test) {
			...
			var self = this;
			var params = {"title":"test will del cat","description":"yo","max_children":0,"enabled":1,"notify_success":"","notify_fail":"","web_hook":"","cpu_limit":0,"cpu_sustain":0,"memory_limit":0,"memory_sustain":0,"session_id":session_id};
		...
		function testAPICreateAPIKey(test) {
			...
			var self = this;
			var params = {"privileges":{"admin":0,"create_events":0,"edit_events":0,"delete_events":1,"run_events":0,"abort_events":0,"state_update":0},"active":"1","title":"test will delete","description":"dshfdwsfs","session_id":session_id};
			...
			request.json( api_url + '/app/create_api_key', params, function(err, resp, data) {
				...
				storage.listFind( 'global/api_keys', { id: data.id }, function(err, api_key) {
					test.ok( !err, "No error fetching data" );
					test.ok( !!api_key, "Data record record is non-null" );
		...
		function testAPIGetActivity(test) {
			...
			async.doWhilst(
				...
				function (err) {
					...
					test.ok( stub.code == 0, "Correct code in activity item" );
					test.ok( stub.event == self.event_id, "Activity item Event ID matches Event ID" );
					test.ok( stub.elapsed > 0, "Activity item has non-zero elapsed time" );
					test.ok( stub.action == "job_complete", "Activity item has correct action" );
		...
		function testWebHookData(test) {
			...
			test.ok( job.id != this.job_id, "Job ID does not match previous job" );
			test.ok( job.code == 0, "Job is not marked as an error" );
