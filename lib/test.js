...
module.exports = {
	...
	tests: [
		...
		function testStorageCLIPasswordLogin(test) {
			// Passwords written by storage-cli.js must be readable by pixl-server-user.
			// This specifically protects the bcryptjs $2b$ compatibility path.
			var username = "cliadmin";
			var password = "cli-password";
			var cli_env = Tools.copyHash( process.env );
			
			// Point the standalone CLI at the unit test storage directory instead of
			// the normal Cronicle data directory from conf/config.json.
			cli_env.CRONICLE_Storage__Filesystem__base_dir = config.Storage.Filesystem.base_dir;
			
			try {
				cp.execFileSync( process.execPath, [
					'bin/storage-cli.js', 'admin', username, password,
					'cliadmin@localhost', '--force', '--quiet'
				], {
					env: cli_env,
					stdio: 'pipe'
				} );
				...
				test.ok( false, "storage-cli.js completed successfully: " + (err.stderr || err.message || err) );
				return test.done();
			}
			
			// Confirm that the CLI created a modern bcrypt hash before exercising
			// the actual login path through pixl-server-user.
			storage.get( 'users/' + username, function(err, user) {
				test.ok( !err, "No error fetching storage-cli user" );
				test.ok( !!user, "storage-cli user record exists" );
				test.ok( !!(user && user.password && user.password.match(/^\$2b\$/)), "storage-cli wrote a bcrypt $2b$ hash" );
				...
				request.json( api_url + '/user/login', params, function(err, resp, data) {
					test.ok( !err, "No error logging in with storage-cli password" );
					test.ok( resp.statusCode == 200, "HTTP 200 from storage-cli user login" );
					test.ok( data.code == 0, "Storage-cli user login succeeded" );
					test.ok( !!data.session_id, "Storage-cli user login returned a session ID" );
		...
		function testAPIEventVisibilityBoundaries(test) {
			// Protected Event and active Job definitions must be filtered before
			// they enter HTTP, login bootstrap, or Socket.IO payloads.
			var self = this;
			var old_active_jobs = null;
			var api_key = {
			...
			var allowed_job = {
				id: 'unitallowedjob',
				event: allowed_event.id,
				event_title: 'Unit Allowed Job',
				category: 'general',
				target: cronicle.server.hostname,
				params: { script: 'UNIT_ALLOWED_SCRIPT' }
			};
			var category_restricted_job = {
				id: 'unitprivatecatjob',
				event: restricted_event.id,
				event_title: 'Unit Category Restricted Job',
				category: restricted_category.id,
				target: 'maingrp',
				params: { script: 'UNIT_PRIVATE_CATEGORY_SCRIPT' }
			};
			var group_restricted_job = {
				id: 'unitprivategrpjob',
				event: 'unitprivategroupevent',
				event_title: 'Unit Group Restricted Job',
				category: 'general',
				target: 'allgrp',
				params: { script: 'UNIT_PRIVATE_GROUP_SCRIPT' }
			};
			...
			async.series([
				...
				function(callback) {
					// Use synthetic in-memory Jobs so this test cannot launch processes.
					old_active_jobs = cronicle.activeJobs;
					cronicle.activeJobs = {};
					cronicle.activeJobs[ allowed_job.id ] = allowed_job;
					cronicle.activeJobs[ category_restricted_job.id ] = category_restricted_job;
					cronicle.activeJobs[ group_restricted_job.id ] = group_restricted_job;
					callback();
				},
				function(callback) {
				...
				function(callback) {
					var params = { api_key: api_key.key };
					request.json( api_url + '/app/get_active_jobs', params, function(err, resp, data) {
						test.ok( !err, "No error requesting limited active Jobs" );
						test.ok( resp.statusCode == 200, "HTTP 200 from limited active Jobs API" );
						test.ok( data.code == 0, "Limited active Jobs request succeeded" );
						test.ok( !!data.jobs[allowed_job.id], "Allowed active Job was returned" );
						test.ok( !data.jobs[category_restricted_job.id], "Category-restricted active Job was filtered from HTTP" );
						test.ok( !data.jobs[group_restricted_job.id], "Group-restricted active Job was filtered from HTTP" );
						test.ok( Tools.numKeys(data.jobs) == 1, "Only one visible active Job was returned" );
						callback();
					} );
				},
				function(callback) {
					...
					request.json( api_url + '/app/get_active_jobs', params, function(err, resp, data) {
						test.ok( !err, "No error requesting administrator active Jobs" );
						test.ok( resp.statusCode == 200, "HTTP 200 from administrator active Jobs API" );
						test.ok( data.code == 0, "Administrator active Jobs request succeeded" );
						test.ok( !!data.jobs[allowed_job.id], "Administrator received allowed active Job" );
						test.ok( !!data.jobs[category_restricted_job.id], "Administrator received category-restricted active Job" );
						test.ok( !!data.jobs[group_restricted_job.id], "Administrator received group-restricted active Job" );
						callback();
					} );
				},
				function(callback) {
				...
				function(callback) {
					...
					cronicle.beforeUserLogin( args, function(err) {
						...
						test.ok( !!args.resp.activeJobs[allowed_job.id], "Allowed active Job was present at login" );
						test.ok( !args.resp.activeJobs[category_restricted_job.id], "Category-restricted active Job was filtered from login" );
						test.ok( !args.resp.activeJobs[group_restricted_job.id], "Group-restricted active Job was filtered from login" );
						callback();
					} );
				},
				function(callback) {
					storage.listGet( 'global/server_groups', 0, 0, function(err, groups) {
						...
						cronicle.sockets = {
							...
							master: {
								_pixl_auth: true,
								_pixl_master: true,
								emit: function(key, data) { master_payload = data; }
							}
						};
						var socket_data = {
							schedule: [allowed_event, restricted_event],
							active_jobs: cronicle.getAllActiveJobs(true)
						};
						cronicle.authSocketEmit( 'status', socket_data, groups );
						cronicle.sockets = old_sockets;
						...
						test.ok( !!limited_payload.active_jobs[allowed_job.id], "Limited socket received allowed active Job" );
						test.ok( !limited_payload.active_jobs[category_restricted_job.id], "Limited socket did not receive category-restricted active Job" );
						test.ok( !limited_payload.active_jobs[group_restricted_job.id], "Limited socket did not receive group-restricted active Job" );
						test.ok( admin_payload.schedule.length == 2, "Administrator socket received all Events" );
						test.ok( !!admin_payload.active_jobs[category_restricted_job.id], "Administrator socket received all active Jobs" );
						test.ok( !unbound_payload, "Socket without a bound user received no schedule" );
						test.ok( master_payload === socket_data, "Master socket received the original cluster status payload" );
						test.ok( !!master_payload.active_jobs[group_restricted_job.id], "Master socket received all active Jobs" );
						test.ok( Tools.numKeys(socket_data.active_jobs) == 3, "Socket filtering did not mutate the source payload" );
						callback();
					} );
				},
				function(callback) {
					cronicle.activeJobs = old_active_jobs;
