...
module.exports = Class.create({
	...
	hasCategoryPrivilege: function(user, cat_id) {
		// Check category access without emitting an API error.  This form is
		// useful when filtering collections before they cross a client boundary.
		if (user.privileges.admin) return true;
		if (!user.privileges.cat_limit) return true;
		return this.hasPrivilege(user, 'cat_' + cat_id);
	},
	
	hasGroupPrivilege: function(args, user, grp_id) {
		// Check group access without emitting an API error.  Targets may be a
		// Server Group ID or an individual hostname covered by an allowed group.
		if (user.privileges.admin) return true;
		if (!user.privileges.grp_limit) return true;
		if (this.hasPrivilege(user, 'grp_' + grp_id)) return true;
		if (!args.server_groups) return false;
		...
		for (var idx = 0, len = groups.length; idx < len; idx++) {
			if (this.hasPrivilege(user, 'grp_' + groups[idx].id)) return true;
		}
		return false;
	},
	
	filterScheduleForUser: function(args, user, schedule) {
		// Return only Events whose Category and target are both visible to the
		// authenticated principal.  Always return a new array for safe emission.
		var self = this;
		return (schedule || []).filter( function(event) {
			return self.hasCategoryPrivilege(user, event.category) &&
				self.hasGroupPrivilege(args, user, event.target);
		} );
	},
	
	filterActiveJobsForUser: function(args, user, jobs) {
		// Return only active Jobs whose Category and target are both visible to
		// the authenticated principal.  Preserve the Job ID keyed hash shape.
		var filtered_jobs = {};
		for (var id in (jobs || {})) {
			var job = jobs[id];
			if (this.hasCategoryPrivilege(user, job.category) && this.hasGroupPrivilege(args, user, job.target)) {
				filtered_jobs[id] = job;
			}
		}
	...
	requireCategoryPrivilege: function(user, cat_id, callback) {
		...
		// otherwise throw an API error and return false
		if (this.hasCategoryPrivilege(user, cat_id)) return true;
	...
	requireGroupPrivilege: function(args, user, grp_id, callback) {
		...
		if (this.hasGroupPrivilege(args, user, grp_id)) return true;
