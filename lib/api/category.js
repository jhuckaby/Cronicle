...
module.exports = Class.create({
	...
	/* api_get_categories: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listGet( 'global/categories', params.offset || 0, params.limit || 50, function(err, items, list) {
				if (err) {
	...
	api_delete_category: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listFind( 'global/schedule', { category: cat.id }, function(err, item) {
				if (item) {
					return self.doError('plugin', "Failed to delete category: Still in use by one or more events.", callback);
				}
				
				self.logDebug(6, "Deleting category: " + cat.title, cat);
				
				self.storage.listFindDelete( 'global/categories', { id: cat.id }, function(err) {
					if (err) {
