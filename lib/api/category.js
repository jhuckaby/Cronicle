...
module.exports = Class.create({
	...
	api_create_category: function(args, callback) {
		...
		if (!this.validateOptionalParams(cat, {
			description: [ 'string', /.*/ ], 
			enabled: [ 'number', /^[1|0]$/ ], 
			max_children: [ 'number', /^\d+$/ ], 
			notify_success: [ 'string', /^[\w\-\@\.\,\s]*$/ ], 
			notify_fail: [ 'string', /^[\w\-\@\.\,\s]*$/ ], 
			web_hook: [ 'string', /(^$|https?\:\/\/\S+)$/i ], 
			cpu_limit: [ 'number', /^\d+$/ ], 
			cpu_sustain: [ 'number', /^\d+$/ ], 
			memory_limit: [ 'number', /^\d+$/ ], 
			memory_sustain: [ 'number', /^\d+$/ ], 
			log_max_size: [ 'number', /^\d+$/ ], 
			color: [ 'string', /^\w+$/ ]
		}, callback)) return;
		...
		if (cat.description && cat.description.match(/[<>]/)) {
