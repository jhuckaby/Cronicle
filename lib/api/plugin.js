...
module.exports = Class.create({
	...
	api_delete_plugin: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listFind( 'global/schedule', { plugin: plugin.id }, function(err, item) {
				if (item) {
					return self.doError('plugin', "Failed to delete plugin: Still assigned to one or more events.", callback);
				}
				
				self.logDebug(6, "Deleting plugin: " + plugin.title, plugin);
				
				// Okay to delete
				self.storage.listFindDelete( 'global/plugins', { id: plugin.id }, function(err) {
					if (err) {
