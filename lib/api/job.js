...
module.exports = Class.create({
	...
	api_get_job_log: function(args, callback) {
		...
		// client API, token auth or standard auth
		var self = this;
		...
		var key = 'jobs/' + query.id + '/log.txt.gz';
		...
		var finish = function() {
			self.storage.getStream( key, function(err, stream) {
				...
				if (query.download) {
					headers['Content-disposition'] = "attachment; filename=Cronicle-Job-Log-" + query.id + '.txt';
				}
		...
		// allow token auth (client UI)
		if (query.t) {
			// auth token validation
			var token = Tools.digestHex(query.id + this.server.config.get('secret_key'));
			if (query.t != token) return this.doError('auth', "Authentication failed.", callback);
			return finish();
		}
		
		// standard auth flow (api key, etc.)
		this.loadSession(args, function(err, session, user) {
			...
			// Standard authentication identifies the caller, but it does not grant
			// access to every Job artifact.  Load the authoritative Job record and
			// enforce the same placement checks used by status and details APIs.
			self.storage.get( 'jobs/' + query.id, function(err, job) {
				if (err) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.head( 'jobs/' + params.id + '/log.txt.gz', function(err, info) {
				...
				self.storage.get( 'jobs/' + params.id, function(err, job) {
					...
					// generate token for viewing/downloading log
					var token = Tools.digestHex(params.id + self.server.config.get('secret_key'));
					
					callback({ code: 0, job: job, token: token });
				} ); // job get
			} ); // log head
		} ); // session
	},
	...
	api_get_active_jobs: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			return callback({
				code: 0,
				jobs: self.filterActiveJobsForUser(args, user, self.getAllActiveJobs())
			});
		} ); // session
	}
