...
module.exports = Class.create({
	...
	api_check_add_server: function(args, callback) {
		...
		if (Math.abs(Tools.timeNow() - params.now) > 60) {
			return this.doError('server', "Server clocks are too far out of sync.");
		}
		if (params.token != Tools.digestHex( params.master + params.now + this.server.config.get('secret_key') )) {
			return this.doError('server', "Secret keys do not match.  Please synchronize your config files.");
		}
	...
	api_get_activity: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listGet( 'logs/activity', params.offset || 0, params.limit || 50, function(err, items, list) {
				if (err) {
