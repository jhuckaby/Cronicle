...
module.exports = Class.create({
	...
	api_create_api_key: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			params.key = Tools.generateUniqueID(32);
	...
	api_update_api_key: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.logDebug(6, "Updating API Key: " + params.title, params);
			
			self.storage.listFindUpdate( 'global/api_keys', { id: params.id }, params, function(err) {
				if (err) {
					return self.doError('api_key', "Failed to update API Key: " + err, callback);
				}
				
				self.logDebug(6, "Successfully updated API Key: " + params.title, params);
				self.logTransaction('apikey_update', params.title, self.getClientInfo(args, { api_key: params }));
				self.logActivity('apikey_update', { api_key: params }, args);
	...
	api_delete_api_key: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.logDebug(6, "Deleting API Key: " + params.title, params);
			
			self.storage.listFindDelete( 'global/api_keys', { id: params.id }, function(err) {
				if (err) {
					return self.doError('api_key', "Failed to delete API Key: " + err, callback);
				}
				
				self.logDebug(6, "Successfully deleted API Key: " + params.title, params);
				self.logTransaction('apikey_delete', params.title, self.getClientInfo(args, { api_key: params }));
				self.logActivity('apikey_delete', { api_key: params }, args);
