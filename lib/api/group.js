...
module.exports = Class.create({
	...
	api_update_server_group: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.logDebug(6, "Updating server group: " + group.title, group);
			
			self.storage.listFindUpdate( 'global/server_groups', { id: group.id }, group, function(err) {
				if (err) {
	...
	api_delete_server_group: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listFind( 'global/schedule', { target: group.id }, function(err, item) {
				if (item) {
					return self.doError('plugin', "Failed to delete server group: Still targeted by one or more events.", callback);
				}
					
				self.logDebug(6, "Deleting server group: " + group.title, group);
				
				self.storage.listFindDelete( 'global/server_groups', { id: group.id }, function(err) {
					if (err) {
